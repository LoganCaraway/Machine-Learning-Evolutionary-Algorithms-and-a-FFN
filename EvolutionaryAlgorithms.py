...
def generateFFNGA(mlp, training_set, validation_set, parameter_struct,
                  prob_cross, prob_mutation, mutation_variance, population_size, max_generations):
    population = initializePopulation(mlp, parameter_struct, population_size)
    population_fitness = evaluateGroupFitness(mlp, training_set, parameter_struct, population)
    for generation in range(1, max_generations):
        print("GA: Working on generation", generation,"of",max_generations,"for",len(parameter_struct)-1,"layer MLP")
        ...
        recombined_fitness = evaluateGroupFitness(mlp, training_set, parameter_struct, recombined_group)
    ...
    population_fitness = evaluateGroupFitness(mlp, validation_set, parameter_struct, population)
...
def initializePopulation(algorithm, parameter_struct, population_size):
...
def evaluateGroupFitness(algorithm, testing_set, parameter_struct, group):
    group_fitness = []
